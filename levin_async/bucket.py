...
from levin_async.exceptions import BadArgumentException
...
class Bucket:
    ...
    @classmethod
    async def from_buffer(cls, signature: c_uint64, sock: asyncio.StreamReader):
        ...
        bucket.cb = await c_uint64.from_buffer(sock)
        bucket.return_data = await c_bool.from_buffer(sock)
        bucket.command = await c_uint32.from_buffer(sock)
        bucket.return_code = await c_int32.from_buffer(sock)
        bucket.flags = await c_uint32.from_buffer(sock)
        bucket.protocol_version = await c_uint32.from_buffer(sock)
        ...
        if (
            bucket.command.value not in P2P_COMMANDS
            and bucket.command.value not in CRYPTONOTE_COMMANDS
        ):
            print("unregonized command: %d" % bucket.command.value)

        bucket.payload = await sock.readexactly(bucket.cb.value)
        ...
        if bucket.command in P2P_COMMANDS:
            log.debug("<< received packet '%s'" % P2P_COMMANDS[bucket.command])
        elif bucket.command in CRYPTONOTE_COMMANDS:
            log.debug("<< received packet '%s'" % CRYPTONOTE_COMMANDS[bucket.command])
        else:
            pass

        from levin_async.reader import LevinReader

        bucket.payload_section = await LevinReader(BytesIO(bucket.payload)).read_payload()
        if bucket.command in P2P_COMMANDS:
            log.debug("<< parsed packet '%s'" % P2P_COMMANDS[bucket.command])
        else:
            log.debug("<< parsed packet '%s'" % CRYPTONOTE_COMMANDS[bucket.command])
        return bucket


    @classmethod
    async def from_buffer_buf(cls, signature: c_uint64, recv_buffer: bytearray):
        if isinstance(signature, bytes):
            signature = c_uint64(signature)
        bucket = cls()
        bucket.signature = signature
