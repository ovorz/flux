...
class _CType:
    ...
    @classmethod
    def from_buffer(cls, buffer, endian: str = 'little', padding: bytes = None, recv_buffer: bytearray = None):
        if isinstance(buffer, BytesIO):
            buffer = buffer.read(cls.NBYTES)
        elif isinstance(buffer, socket.socket):
            # 如果提供了recv_buffer，则从其中读取数据
            if recv_buffer is not None:
                if len(recv_buffer) < cls.NBYTES:
                    return None
                buffer = recv_buffer[:cls.NBYTES]  # 使用切片获取数据
            else:
                # 如果socket没有recv_buffer属性，说明是其他调用场景
                try:
                    buffer = buffer.recv(cls.NBYTES, socket.MSG_DONTWAIT)
                except BlockingIOError:
                    return None
                except Exception as e:
                    raise BadArgumentException(f"Error reading from socket: {str(e)}")
