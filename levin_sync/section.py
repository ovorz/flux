...
from levin_sync.utils import ip2int
...
class Section:
    ...
    def from_byte_array(cls, buffer: BytesIO):
        from levin_sync.reader import LevinReader
    ...
    @classmethod
    def handshake_request(cls, my_port: int, network_id: bytes, peer_id: bytes, block_height: int = 0):
        ...
        payload_data.add("cumulative_difficulty", c_uint64(monero_data['cumulative_difficulty']))
        payload_data.add("cumulative_difficulty_top64", c_uint64(monero_data['cumulative_difficulty_top64']))
        payload_data.add("current_height", c_uint64(monero_data['height'] + 1))
        payload_data.add("top_id", c_string(genesis_hash))
        payload_data.add("top_version", c_ubyte(16))
        section.add("payload_data", payload_data)
        return section

    @classmethod
    def handshake_response(
        cls, my_port: int, network_id: bytes, peer_id: bytes, peerlist: list, block_height: int = 0
    ):
        if network_id == NETWORK_ID_MAINNET:
            genesis_hash = GENESIS_HASH_MAINNET
        elif network_id == NETWORK_ID_TESTNET:
    ...
    @classmethod
    def timed_sync_request(cls, network_id: bytes, peer_id: bytes, block_height: int = 0):
        if network_id == NETWORK_ID_MAINNET:
            genesis_hash = GENESIS_HASH_MAINNET
        elif network_id == NETWORK_ID_TESTNET:
            genesis_hash = GENESIS_HASH_TESTNET
    ...
    def __bytes__(self):
        from levin_sync.writer import LevinWriter
